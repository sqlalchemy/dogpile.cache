...
class BackendArgsAntiMutationTest:
    """
    This test ensures backends arguments are not mutated
    """

    # note each backend as it's tested here
    _backends_tested = []

    @classmethod
    def teardown_class(cls):
        """
        this is a compliance test. it will cause an Error in the test suite if:
        * any test fails
        * a new backend is registered, but not tested
        """
        backends_missing = []
        _backends_eligible = [
            k
            for k, v in _backend_loader._unloaded.items()
            if k.startswith("dogpile.cache.")
        ]
        for bck in _backends_eligible:
            if bck not in cls._backends_tested:
                backends_missing.append(bck)
        assert not backends_missing

    def _test_arguments(self, backend: str, arguments: Dict):
        backend_cls = _backend_loader.load(backend)
        arguments_copy = arguments.copy()
        _backend_inst = backend_cls(arguments_copy)  # noqa: F841
        assert arguments_copy == arguments

        # note this backend has been tested;
        # used by `BackendArgsAntiMutationTest.teardown_class`
        BackendArgsAntiMutationTest._backends_tested.append(backend)

    def test_null(self):
        backend = "dogpile.cache.null"
        arguments = {}
        self._test_arguments(backend, arguments)

    def test_dbm(self):
        from .test_dbm_backend import test_fname

        backend = "dogpile.cache.dbm"
        arguments = {"filename": test_fname}
        self._test_arguments(backend, arguments)

    def test_pylibmc(self):
        from .test_memcached_backend import MEMCACHED_URL

        backend = "dogpile.cache.pylibmc"
        arguments = {"url": MEMCACHED_URL}
        self._test_arguments(backend, arguments)

    def test_bmemcached(self):
        from .test_memcached_backend import MEMCACHED_URL
    ...
    def test_redis_sentinel(self):
        from .test_redis_sentinel_backend import REDIS_HOST
        from .test_redis_sentinel_backend import REDIS_PORT
    ...
    def test_redis_cluster(self):
        ...
        backend = "dogpile.cache.redis_cluster"
        arguments = {
            "startup_nodes": [
    ...
    def test_valkey(self):
        from .test_valkey_backend import VALKEY_HOST
        from .test_valkey_backend import VALKEY_PORT
    ...
    def test_valkey_sentinel(self):
        from .test_valkey_sentinel_backend import VALKEY_HOST
        from .test_valkey_sentinel_backend import VALKEY_PORT
    ...
    def test_valkey_cluster(self):
        ...
        backend = "dogpile.cache.valkey_cluster"
        arguments = {
            "startup_nodes": [
                ClusterNode(VALKEY_HOST, VALKEY_PORT),
            ],
        }
        self._test_arguments(backend, arguments)
